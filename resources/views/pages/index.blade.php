        ...
        <div class="container position-relative text-white z-index-1" data-aos="fade-up">
            <div class="row">
                <div class="col-lg-8 col-md-10 bg-dark bg-opacity-50 p-4 p-lg-5 rounded-4 glass-effect">
                    <h1 class="display-3 fw-bold mb-3 text-white">{{ $settings['home_intro_title'] ?? 'Trà Việt Nam là nghệ sĩ' }}</h1>
                    <div class="lead mb-4 text-white">{!! $settings['home_intro_desc'] ?? 'Chúng tôi luôn đặt chất lượng và an toàn thực phẩm lên hàng đầu trong mọi quy trình sản xuất. Toàn bộ hệ thống nhà xưởng và quy trình chế biến được đảm bảo đạt chuẩn ISO 22000:2018.' !!}</div>
                    <a href="{{ route('about') }}" class="btn btn-success btn-lg px-4 rounded-pill shadow">{{ app()->getLocale() == 'en' ? 'Discover now' : 'Khám phá ngay' }}</a>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container py-4 position-relative z-index-1">
            <div class="row align-items-center g-5">
                ...
                <div class="col-lg-6" data-aos="fade-left">
                    ...
                    <h4 class="fw-bold mb-4 text-success opacity-75">{{ $settings['home_about_title'] ?? 'Giới thiệu Kim Thành' }}</h4>
                    <div class="text-muted fs-5 mb-4">
                        {!! $settings['home_about_desc'] ?? 'Công ty TNHH Đầu tư Thương mại Kim Thành được thành lập vào ngày 03/02/2020...' !!}
                    </div>
                    <a href="{{ route('about') }}" class="btn btn-outline-success btn-lg rounded-pill px-4">{{ app()->getLocale() == 'en' ? 'View details' : 'Xem chi tiết' }} <i
    ...
    <section id="home-products" class="py-5">
        <div class="container py-4">
            <div class="text-center mb-2" data-aos="fade-up">
                <h2 class="display-4 text-success text-uppercase fw-bold mb-2">{{ app()->getLocale() == 'en' ? 'Featured Products' : 'Sản phẩm nổi bật' }}</h2>
            </div>

            @php
                $groupedProducts = $products->groupBy(function($item) {
                    return $item->category ? $item->category->getTranslation('name', app()->getLocale()) : (app()->getLocale() == 'en' ? 'Others' : 'Khác');
                });
            @endphp
            ...
            <div class="text-center mt-5" data-aos="fade-up">
                <a href="{{ route('products') }}" class="btn btn-outline-success btn-lg rounded-pill px-5">{{ app()->getLocale() == 'en' ? 'View all products' : 'Xem tất cả sản phẩm' }}</a>
            </div>
        </div>
    </section>
