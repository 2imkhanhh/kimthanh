    ...
    <section id="panel-products" class="py-5 bg-light">
        <div class="container py-4">
            ...
            <ul class="nav nav-pills justify-content-center mb-5 gap-3" id="productTabs" data-aos="fade-up">
                <li class="nav-item">
                    <a href="{{ route(app()->getLocale() . '.products') }}" class="nav-link {{ !$activeCategoryId ? 'active' : 'bg-white text-secondary' }} px-4 py-2 rounded-pill shadow-sm">
                ...
                @if(isset($categories) && $categories->count() > 0)
                    ...
                    <li class="nav-item">
                        <a href="{{ route(app()->getLocale() . '.products', ['category' => $cat->id]) }}" class="nav-link {{ $activeCategoryId == $cat->id ? 'active' : 'bg-white text-secondary' }} px-4 py-2 rounded-pill shadow-sm">
                        ...
                        <div class="col" data-aos="fade-up" data-aos-delay="{{ ($index % 3 + 1) * 100 }}">
                            <div class="card h-100 border-0 shadow-sm product-card transition-all">
                                ...
                                <div class="card-body text-center p-4">
                                    ...
                                    <a href="{{ route(app()->getLocale() . '.contact') }}" class="btn btn-success rounded-pill px-4 py-2 w-100">{{ app()->getLocale() == 'en' ? 'Contact for price' : 'Liên hệ báo giá' }}</a>
                                </div>
                            </div>
                        </div>
                        @endforeach
